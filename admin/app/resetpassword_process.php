...
function createOrUpdateVerification($trainmastas_conn, $UID, $email, $verification_code, $action_type)
{
    ...
    if ($email === null) {
        $query = "SELECT a.`Email` FROM `admin` AS a WHERE a.`user_ID` = ?";
        $temp_stmt = $trainmastas_conn->prepare($query);
        $temp_stmt->bind_param("s", $UID);
        $temp_stmt->execute();
        $temp_stmt->bind_result($email);
    } else {
        $query = "SELECT a.`user_ID` FROM `admin` AS a WHERE a.`Email` = ?";
        $temp_stmt = $trainmastas_conn->prepare($query);
    ...
    if ($record_found) {
        ...
        if ($status === "pending" && $attempt_count < 3) {
            ...
            if (strtotime($updated_at) < strtotime($fifteen_minutes_ago)) {
                $attempt_count++;
                $query = "UPDATE `user_verification` 
        ...
        } elseif ($status === "pending" && $attempt_count >= 3 && strtotime($updated_at) > strtotime($six_hours_ago)) {
            return [
                "state" => "limitReached",
...
function verifyCode($trainmastas_conn, $UID, $email, $verification_code)
{
    ...
    if ($email === null) {
        ...
        $query = "SELECT a.`Email` FROM `admin` AS a WHERE a.`user_ID` = ?"; // Assuming user_ID is the correct field in the admin table  
        $temp_stmt = $trainmastas_conn->prepare($query);
        ...
        $query = "SELECT a.`user_ID`  FROM `admin` AS a WHERE a.`Email` = ?"; // Fetch user_ID using the provided email  
...
function updatePassword($trainmastas_conn, $UID, $email, $password)
{
    ...
    $query = "UPDATE `admin` SET `Password` = ? WHERE `$addQuery` = ?";
    $stmt = $trainmastas_conn->prepare($query);
...
if (isset($_POST['purpose']) && $_POST['purpose'] === "verifyPasswordAction") {
    ...
    if ($UID) {
        $query = "SELECT password FROM admin WHERE user_ID = ?";
        $stmt = $trainmastas_conn->prepare($query);
    ...
    // Fetch stored password hash if the user_ID is present
    if ($bindThis) {
        $query = "SELECT password FROM admin WHERE $addQuery = ?";
        $stmt = $trainmastas_conn->prepare($query);
    ...
    $password = $_POST['password'];
    if (isset($_SESSION['AID'])) {
        $UID = $_SESSION['AID']; // Replace with actual session or user ID
    } else {
        header("location:login.php");
    }
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($stmt = $trainmastas_conn->prepare($query)) {
                ...
                if ($count > 0) {
                    ...
                    $response = [
                        "state" => "not_found",
                        "message" => "Email does not exist in the admin table."
                    ];
                }
